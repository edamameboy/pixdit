...
export async function saveGeneratedImage({ generatedRoot, jobId, index, result }) {
  const safeJobId = String(jobId).replace(/[^a-zA-Z0-9-]/g, "");
  const folder = path.join(generatedRoot, safeJobId);
  await fs.mkdir(folder, { recursive: true });
  const fileName = `concept-${String(index + 1).padStart(2, "0")}.${result.extension}`;
  await fs.writeFile(path.join(folder, fileName), result.buffer, { mode: 0o600 });
  return `/generated/${safeJobId}/${fileName}`;
}
